...
#include <veins/modules/application/traci/SmartJunctionRSUApp.h>
#include "veins/modules/application/traci/TraCIDemo11pMessage_m.h"
#include <algorithm>
#include <initializer_list>
using namespace veins;

Define_Module(veins::SmartJunctionRSUApp);

void SmartJunctionRSUApp::onWSA(DemoServiceAdvertisment* wsa)
{
...
void SmartJunctionRSUApp::onWSM(BaseFrame1609_4* frame)
{
    ...
    if (counter_max == 8 && sentMessage == false) { //send message to vehicles on most free road saying to slow down
        ...
        if (counter_bottom == counter_min) {
            message = "bottom road slow down";

        } else if (counter_left == counter_min){
            message = "left road slow down";

      //  } else if (counter_right == counter_min) {
        //    message = "right road slow down";
//
       // } else if (counter_top == counter_min) {
       //     message = "top road slow down";
        }
...
void SmartJunctionRSUApp::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind()) {
