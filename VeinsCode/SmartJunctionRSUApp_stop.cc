...
#include <veins/modules/application/traci/SmartJunctionRSUApp_stop.h>
#include "veins/modules/application/traci/TraCIDemo11pMessage_m.h"
#include <algorithm>
#include <initializer_list>
using namespace veins;

Define_Module(veins::SmartJunctionRSUApp_stop);

void SmartJunctionRSUApp_stop::onWSA(DemoServiceAdvertisment* wsa)
{
...
void SmartJunctionRSUApp_stop::onWSM(BaseFrame1609_4* frame)
{
    ...
    if (counter_max == 5 && sentMessage == false) { //send message to vehicles on most free road saying to slow down
        ...
        if (counter_left == counter_max) {
            message = "all roads apart from left road stop";

        } else if (counter_bottom == counter_max){
...
void SmartJunctionRSUApp_stop::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind()) {
